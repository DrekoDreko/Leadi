      ...
      <section className="glass-strong rounded-[34px] p-5 md:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-2 rounded-full bg-surface-elevated px-4 py-2 text-sm font-semibold">
            ...
            {planProducts.map((product) => (
              <article
                className={`rounded-[26px] border p-4 ${
                  product.featured ? "border-cobalt/30 bg-cobalt/8" : "border-border bg-surface-elevated"
                }`}
                key={product.key}
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {product.badge ? (
                        <span className="rounded-full bg-surface-elevated px-2.5 py-1 text-[11px] font-semibold uppercase tracking-normal text-ink/54">
                          {product.badge}
                        </span>
                      ) : null}
                    </div>
        ...
        <div className="glass-strong rounded-[34px] p-5 md:p-6">
          ...
          <div className="grid gap-3">
            {packProducts.map((product) => (
              <article
                className={`rounded-[26px] border p-4 ${
                  product.featured ? "border-lagoon/30 bg-lagoon/8" : "border-border bg-surface-elevated"
                }`}
                key={product.key}
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          {snapshot.recentTransactions.length > 0 ? (
            <div className="space-y-3">
              {snapshot.recentTransactions.map((transaction) => (
                <div className="rounded-[22px] bg-surface-elevated p-4" key={transaction.id}>
          ...
          {snapshot.recentPurchases.length > 0 ? (
            <div className="space-y-3">
              {snapshot.recentPurchases.map((purchase) => {
                ...
                return (
                  <div className="rounded-[22px] bg-surface-elevated p-4" key={purchase.id}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                      ...
                      <span className="rounded-full bg-surface-elevated px-3 py-1.5 text-xs font-semibold text-ink">
