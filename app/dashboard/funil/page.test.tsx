...
vi.mock("@/lib/pipeline/board.server", () => ({
  getSalesBoardForCurrentUser: vi.fn()
}));

vi.mock("@/lib/pipeline/cards.server", () => ({
  listLabelsForCurrentUser: vi.fn()
}));
...
vi.mock("./sales-funnel-workspace", () => ({
  SalesFunnelWorkspace: (props: any) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
    <div data-testid="sales-funnel-workspace">
      ...
      <span>Stages count: {props.stages.length}</span>
...
describe("Sales Funnel Page (/dashboard/funil)", () => {
  it("entrega board, saldo e templates de WhatsApp ao workspace do funil", async () => {
    vi.mocked(requireCompletedProfile).mockResolvedValue({ isManager: true, isOwner: false } as never);
    vi.mocked(parseLeadUrlFilters).mockReturnValue({} as never);
    vi.mocked(getSalesBoardForCurrentUser).mockResolvedValue({
      leadState: { leads: [{ id: "lead-1" }] },
      stages: [{ id: "stage-1" }, { id: "stage-2" }]
    } as never);
    vi.mocked(listLeadOwnerOptionsForCurrentUser).mockResolvedValue([] as never);
    vi.mocked(listLabelsForCurrentUser).mockResolvedValue([] as never);
    vi.mocked(getCurrentResourceAccess).mockResolvedValue({ allowed: true } as never);
    ...
    expect(screen.getByText(/Stages count: 2/i)).toBeInTheDocument();
