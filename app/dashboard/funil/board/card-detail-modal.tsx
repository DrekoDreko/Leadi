...
export function CardDetailModal({
  ...
  return createPortal(
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-[70] flex items-start justify-center overflow-y-auto bg-black/60 p-4 backdrop-blur-md md:p-8"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        onClick={onClose}
      >
        <motion.div
          className="surface-modal relative my-auto w-full max-w-2xl rounded-3xl p-5 md:p-6"
        ...
        >
          <button
            className="icon-button absolute right-4 top-4 h-9 w-9 bg-surface-elevated/88"
            onClick={onClose}
            aria-label="Fechar"
            type="button"
          >
