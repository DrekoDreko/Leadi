...
- [x] **`ABACATE_PAY_WEBHOOK_SECRET` em produção** — ✅ definido na Vercel (`production`) em 25/06/2026 e espelhado no `.env.local`. O webhook valida HMAC-SHA256 do corpo com este segredo (header `x-webhook-signature`). ⚠️ **Pendência de espelhamento:** o **mesmo** valor precisa estar configurado no painel do AbacatePay (webhook) — sem isso a assinatura não bate quando o billing real for ligado. Como `BILLING_DISABLED=true`, ainda não há webhooks reais; confirmar o lado do painel antes de reativar billing.
